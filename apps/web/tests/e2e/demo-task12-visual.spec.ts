...
test("touched surfaces remain polished at every required viewport", async ({ page }) => {
  ...
  for (const viewport of viewports.filter(
    ...
    await expect(page.locator(".goalDraftText")).toContainText("세\u00a0쪽으로")
    const listingAction = page.getByRole("button", { name: "이 목표 상장하기" })
    await listingAction.scrollIntoViewIfNeeded()
    await expect(listingAction).toBeInViewport({ ratio: 1 })
    await capture(page, viewport.label, "analysis-result", false)
    ...
    const reset = page.getByRole("dialog", { name: "모든 데이터를 초기화할까요?" })
    ...
    await page.getByRole("heading", { name: "사용 가능 1개" }).scrollIntoViewIfNeeded()
