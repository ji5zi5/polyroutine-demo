...
import { assertNoCriticalAxeViolations, captureResponsiveState } from "./support/visual"
...
test("evidence-capture rejects a server-expired challenge without offering transport retry", async ({
  ...
  // Then
  await expect(page.getByRole("alert").filter({ hasText: "10분 코드가 만료됐어요" })).toBeVisible()
...
test("evidence-capture reports the server deadline without storing late bytes", async ({
  ...
  // Then
  await expect(
    page.getByRole("alert").filter({ hasText: "증거 제출 마감이 지났어요" }),
  ).toBeVisible()
  await expect(page.getByRole("button", { name: "접수 여부 다시 확인하기" })).toHaveCount(0)
...
test("evidence-capture permits one processing resubmission and stops after attempt two", async ({
  ...
  await page.getByRole("button", { name: "검토 상태 확인하기" }).click()
  await expect(
    page.getByText("흐림·빛 반사·잘림 때문에 안내 항목을 확인하기 어려웠어요"),
  ).toBeVisible()
  await page.getByRole("button", { name: "새 코드로 다시 제출하기" }).click()
  ...
  // Then
  await expect(
    page.getByText("두 번의 제출 기회를 모두 사용했어요", { exact: false }),
  ).toBeVisible()
  await expect(page.getByRole("button", { name: "새 코드로 다시 제출하기" })).toHaveCount(0)
  await captureResponsiveState(page, "evidence-result-exhausted")
  await assertNoCriticalAxeViolations(page, "evidence-result-exhausted")
})
